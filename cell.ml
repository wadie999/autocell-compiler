...
let invoke vm state d a b =
	...
	with Invalid_argument _ ->
		...
		if x < 0 || x >= w then raise (Vm.Error (vm, sprintf "x = %d out of bounds [0, %d]" x (w - 1)))
		else if y < 0 || y >= h then raise (Vm.Error (vm, sprintf "y = %d out of bounds [0, %d]" y (h - 1)))
		else raise (Vm.Error (vm, sprintf "field = %d out of bounds [0, %d]" b (d - 1)))
